...
export default function Home() {
  ...
  return <main className="page">
    <section className="hero"><div className="badge">● LOW-PC · CLOUD-FIRST · OPEN SOURCE</div><h1>Lazarius <span>Video AI</span></h1><p>Décris une idée. Ajoute une image de référence si tu veux garder un personnage cohérent. Lazarius orchestre scénario → vidéo → voix → sous-titres → montage → MP4.</p></section>
    <section className="card">
      ...
      <input value={characterDescription} onChange={e => setCharacterDescription(e.target.value)} maxLength={1000} placeholder="Ex. Homme ivoirien de 25 ans, cheveux courts, casque jaune, gilet orange, pantalon bleu..." />
      <p className="runtime">Le verrou d’identité répète ces caractéristiques dans les prompts de toutes les scènes.</p>
      ...
      {imageUrl.trim() && <p className="runtime">Mode image → vidéo activé : l’image sert de référence de départ et peut être propagée entre les scènes.</p>}
      <label className="check"><input type="checkbox" checked={chainScenes} onChange={e => setChainScenes(e.target.checked)} disabled={!imageUrl.trim()} /> Chaîner les scènes : dernière image d’une scène → première image de la suivante</label>
      ...
      <div className="controls">
        ...
        <button onClick={generate} disabled={loading || !prompt.trim() || uploading}>{loading ? "Lancement..." : imageUrl.trim() ? "Créer la vidéo cohérente →" : "Créer la vidéo →"}</button>
      </div>
